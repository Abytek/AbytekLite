...
{
	/**
	 * @brief A fixed-size push-only queue data structure
	 * 
	 * This container provides a queue with a maximum capacity where items
	 * can only be pushed (not popped). It's optimized for sequential access
	...
	public:
		...
		/**
		 * @brief Check if the queue has been initialized
		 * @return True if the queue has not been initialized, false otherwise
		 */
		...
		/**
		 * @brief Get a span of the items in the queue (const version)
		 * @return Constant span of queue items
		 */
		ABYTEK_FORCE_INLINE boost::span<const F_Item> GetItemSpan() const noexcept
		{
			return { _ItemVector.data(), _EndIndex };
		}
		
		/**
		 * @brief Get a span of the items in the queue
		 * @return Span of queue items
		 */
	...
	public:
		 ...
		 */
		ABYTEK_FORCE_INLINE TF_FixedPOQueue() noexcept = default;
		 ...
		 */
		TF_FixedPOQueue(Sz Capacity, const F_Allocator& Allocator) :
			_Capacity(Capacity),
		 ...
		 * @param Capacity The maximum number of items the queue can hold
		 */
		TF_FixedPOQueue(Sz Capacity) :
			TF_FixedPOQueue(Capacity, {})
		{
		}
		 ...
		 */
		TF_FixedPOQueue(const TF_FixedPOQueue& x) :
		 ...
		 */
		TF_FixedPOQueue& operator = (const TF_FixedPOQueue& x)
		{
			_ItemVector = x._ItemVector;
			_Capacity = x._Capacity;
			_EndIndex = x._EndIndex;
		 ...
		 */
		TF_FixedPOQueue(TF_FixedPOQueue&& x) :
		 ...
		 * @return Reference to this object
		 */
		TF_FixedPOQueue& operator = (TF_FixedPOQueue&& x)
		{
			_ItemVector = std::move(x._ItemVector);
			_Capacity = x._Capacity;
			_EndIndex = x._EndIndex;
	...
	private:
		...
		Sz _Push(__F_PassedItem&& item)
		{
			...
			Sz Location = _EndIndex;

			++_EndIndex;
			_ItemVector.resize(_EndIndex);

			ABYTEK_BASE_CORE_ASSERT(_EndIndex <= _Capacity) << "Capacity exceeded";

			_ItemVector[Location] = std::forward<__F_PassedItem>(item);
	...
	public:
		/**
		 * @brief Push a copy of an item into the queue
		 * @param item The item to push
		 * @return The index where the item was placed
		 */
		...
		/**
		 * @brief Push an item into the queue using move semantics
		 * @param item The item to push
		 * @return The index where the item was placed
		 */
		 ...
		 * Removes all items and resets the end index to 0
		 */
