...
{
	/**
	 * @brief A thread-safe fixed-size push-only queue data structure
	 * 
	...
	class TF_ConcurrentFixedPOQueue
	{
	public:
	...
	public:
		...
		{
			return { _ItemVector.data(), GetEndIndex() };
		}
	...
	public:
		 ...
		 */
		ABYTEK_FORCE_INLINE TF_ConcurrentFixedPOQueue() noexcept = default;
		 ...
		 */
		TF_ConcurrentFixedPOQueue(Sz Capacity, const F_Allocator& Allocator) :
			_Capacity(Capacity),
		 ...
		 * @param Capacity The maximum number of items the queue can hold
		 */
		TF_ConcurrentFixedPOQueue(Sz Capacity) :
			TF_ConcurrentFixedPOQueue(Capacity, {})
		{
		}
		 ...
		 */
		TF_ConcurrentFixedPOQueue(const TF_ConcurrentFixedPOQueue& x) :
		 ...
		 */
		TF_ConcurrentFixedPOQueue& operator = (const TF_ConcurrentFixedPOQueue& x)
		{
			_ItemVector = x._ItemVector;
			_Capacity = x._Capacity;
			_EndIndex = x.GetEndIndex();
		 ...
		 */
		TF_ConcurrentFixedPOQueue(TF_ConcurrentFixedPOQueue&& x) :
		 ...
		 * @return Reference to this object
		 */
		TF_ConcurrentFixedPOQueue& operator = (TF_ConcurrentFixedPOQueue&& x)
		{
			_ItemVector = std::move(x._ItemVector);
			_Capacity = x._Capacity;
			_EndIndex = x.GetEndIndex();
	...
	private:
		...
		{
			Sz Location = _EndIndex.fetch_add(1, boost::memory_order_relaxed);
