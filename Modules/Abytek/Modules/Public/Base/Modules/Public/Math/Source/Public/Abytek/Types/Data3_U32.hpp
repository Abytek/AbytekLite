...
{
    /**
     * @brief A 3D vector with U32 (unsigned int) elements
     * 
    ...
    struct TF_Data<3, 1, U32, __F_Config> : __F_Config::template TA_DefaultData<3, 1, U32, __F_Config>
    {
         ...
         */
        ABYTEK_FORCE_INLINE F_Element& operator [] (U32 Idx) & noexcept
        {
         ...
         */
        ABYTEK_FORCE_INLINE F_Element operator [] (U32 Idx) const & noexcept
        {
            ABYTEK_BASE_MATH_ASSERT(Idx < RowCount) << "Idx out of bounds";
            return Elements[Idx];
        }
    ...
    template<class __F_Config>
    ABYTEK_REFLECT(TF_Data<3, 1, U32, __F_Config>)
    {
