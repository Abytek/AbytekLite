...
/// @file Data4_I16.hpp
/// @brief Contains the specialized template definition for 4-dimensional unsigned integer data

namespace Abytek
{
    /// @brief Specialized template for 4-dimensional unsigned integer vectors
    /// @tparam __F_Config Configuration structure for the data type
    template<class __F_Config>
    struct TF_Data<4, 1, I16, __F_Config> : __F_Config::template TA_DefaultData<4, 1, I16, __F_Config>
    {
        using F_This = TF_Data;
        ABYTEK_BEGIN_REFLECTOR(I_Serializable)
        ABYTEK_END_REFLECTOR(F_This)
        
        static constexpr I16 RowCount = 4;
        static constexpr I16 ColumnCount = 1;
        ...
        ABYTEK_FORCE_INLINE F_Element& operator [] (I16 Idx) & noexcept
        {
        ...
        ABYTEK_FORCE_INLINE F_Element operator [] (I16 Idx) const & noexcept
        {
                        ABYTEK_BASE_MATH_ASSERT(Idx < RowCount) << "Idx out of bounds";
            return Elements[Idx];
        }
    ...
    template<class __F_Config>
    ABYTEK_REFLECT(TF_Data<4, 1, I16, __F_Config>)
    {
