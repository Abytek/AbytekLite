...
namespace Abytek
{
    ...
    struct TF_Data<4, 1, U8, __F_Config> : __F_Config::template TA_DefaultData<4, 1, U8, __F_Config>
    {
        using F_This = TF_Data;
        ABYTEK_BEGIN_REFLECTOR(I_Serializable)
        ABYTEK_END_REFLECTOR(F_This);
        
        static constexpr U8 RowCount = 4;
        static constexpr U8 ColumnCount = 1;
        ...
        ABYTEK_FORCE_INLINE F_Element& operator [] (U8 Idx) & noexcept
        {
        ...
        ABYTEK_FORCE_INLINE F_Element operator [] (U8 Idx) const & noexcept
        {
                        ABYTEK_BASE_MATH_ASSERT(Idx < RowCount) << "Idx out of bounds";
            return Elements[Idx];
        }
    ...
    template<class __F_Config>
    ABYTEK_REFLECT(TF_Data<4, 1, U8, __F_Config>)
    {
