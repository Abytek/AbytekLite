#include "Abytek/DirectX12/RHIClearUAVUIntPass.hpp"
#include "Abytek/DirectX12/RHIResourceView.hpp"


#ifdef ABYTEK_ENGINE_RHI_ENABLE_DIRECTX12
namespace Abytek
{
    void F_DirectX12RHIClearUAVUIntPass::Build(const F_RHIClearUAVUIntPassBuildParams& BuildParams)
    {
        A_RHIClearUAVUIntPass::Build(BuildParams);
        A_DirectX12RHIPassExtension::Build();
    }   
    void F_DirectX12RHIClearUAVUIntPass::Release()
    {
        A_DirectX12RHIPassExtension::Release();
        A_RHIClearUAVUIntPass::Release();
    }

    void F_DirectX12RHIClearUAVUIntPass::AppendSubresourceBindings(F_DirectX12RHISubresourceBindingSet& SubresourceBindingSet)
    {
        A_DirectX12RHIPassExtension::AppendSubresourceBindings(SubresourceBindingSet);
        
        GetUAV().FastCast<F_DirectX12RHIResourceView>()->AppendSubresourceBindingSet(SubresourceBindingSet);
    }
}
#endif